...
def cfg():
    ...
    parser.add_argument('--start_iter', type=int, default=1371, help='Starting Epoch')
...
def eval(model, epoch, Peak_psnr, Peak_epoch, Peak_ssim, Peak_ssim_epoch):
    print("==> Start testing")
    tStart = time.time()
    ...
    if ssim_score > Peak_ssim:
        Peak_ssim = ssim_score
        Peak_ssim_epoch = epoch

    return psnr_score, ssim_score, Peak_psnr, Peak_epoch, Peak_ssim, Peak_ssim_epoch
...
def main(opt, _run):
    ...
    train_set = get_training_set("datasets/totalLOL/train/1625", 1, opt.patch_size, True) # uncomment it to do the fine tuning
    ...
    lighten.load_state_dict(torch.load('models/DLN_1370.pth', map_location=lambda storage, loc: storage), strict=True)
    ...
    history = {'psnr': [],
               'ssim': []}
    psnr_score, ssim_score, Peak_psnr, Peak_epoch, Peak_ssim, Peak_ssim_epoch = eval(lighten, 0, Peak_psnr, Peak_epoch, Peak_ssim, Peak_ssim_epoch)
    print(psnr_score)
    for epoch in range(opt.start_iter, opt.nEpochs + 1):
        ...
        for iteration, batch in enumerate(training_data_loader, 1):
            ...
            if iteration % 10 == 0:
                ...
                # print("Peak_psnr : %2.4f Peak_epoch : %03d" % (Peak_psnr, Peak_epoch))
        ...
        if epoch % (opt.snapshots) == 0:
            ...
            psnr_score, ssim_score, Peak_psnr, Peak_epoch, Peak_ssim, Peak_ssim_epoch= eval(lighten, epoch, Peak_psnr, Peak_epoch, Peak_ssim, Peak_ssim_epoch)
            ...
            logs = {
                ...
                "pepoch_P": Peak_epoch,
                "peak_S": Peak_ssim,
                "pepoch_S": Peak_ssim_epoch,
            }
            log_metrics(_run, logs, epoch, end_str="\n")
            '''
            fig1 = plt.figure()
            plt.plot(range(epoch), history['psnr'], label='psnr', color='darkred')
            plt.xlabel('epochs')
            plt.ylabel('psnr')
            ...
            plt.close(fig1)
            fig2 = plt.figure()
            plt.plot(range(epoch), history['ssim'], label='ssim', color='darkred')
            plt.xlabel('epochs')
            plt.ylabel('ssim')
            plt.grid(linestyle='--', color='lavender')
            # plt.show()
